/* File header: This file contains funciton for combining sequences in sorted order.
 * Student Name: Vasu Patel
*/
...
/** Helper function to merge two subSequences
 * @brief mergeSubSequences: merges two sub-sequences, namely: left and right
 * @param leftSubSequence
 * @param rightSubSequence
 * @return a vector of DataPoints sorted in ascending order of weights from left and right subSequences.
 */
Vector<DataPoint> mergeSubSequences(const Vector<DataPoint>& left,
                                    const Vector<DataPoint>& right) {
    Vector<DataPoint> merged;
    int i = 0, j = 0;
    // check for lowest value from left and right subSequence and store it in merged vector
    ...
    // if left subSequence is bigger than right, keep adding values from left subSequence
    while (i < left.size()) {
        merged.add(left[i]);
        i++;
    }
    // if right subSequence is bigger than left, keep adding values from right subSequence
    while (j < right.size()) {
        merged.add(right[j]);
        j++;
    }
...
/** Recusive function to comvbine vector of sequences
 * @brief combine
 * @param sequences
 * @return a vector of DataPoints sorted in ascending order of weights from provided vector of sequences
 */
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    Vector<DataPoint> combinedSequences;
    // Sanity check for empty sequences; returns empty vector
    if (sequences.size() == 0) {
        return {};
    }
    // Base case: only 1 sequence in the provided sequences, answer will be the input sequence itself
    if (sequences.size() == 1) {
        return sequences[0];
    }

    // find mid-index for the
    int half = sequences.size() / 2;
    Vector<Vector<DataPoint>> leftSubSequence = sequences.subList(0, half);
    Vector<Vector<DataPoint>> rightSubSequence = sequences.subList(half);

    // Recursively split list of sequences into 2 sub-sequences: left and right
    Vector<DataPoint> left = combine(leftSubSequence);
    Vector<DataPoint> right = combine(rightSubSequence);

    // merge left and right subSequences
    return mergeSubSequences(left, right);
}


/* * * * * * Test Cases Below This Point * * * * * */

STUDENT_TEST("Merges two single-item sequences with same weight.") {
    DataPoint amy        = { "Amy Liu",          0 };
    DataPoint katherine  = { "Katherine Erdman", 0 };
    ...
    EXPECT_EQUAL(merged.size(), 2);
    EXPECT_EQUAL(merged[0], katherine);
    EXPECT_EQUAL(merged[1], amy);
}
