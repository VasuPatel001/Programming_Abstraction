/* File header: this file contains the code for priority queue using heaps.
*/
 ...
 * @brief HeapPQueue::bubbleUp: tests if an added element can be bubbled up
 * @param elems: pointer to array
 * @param childNode: current index of newly added element
 * @param parentNode: index of element present above in the Heap to childNode
 */
void HeapPQueue::bubbleUp(DataPoint*& elems,
                          int childNode,
                          int parentNode) {
    // Base case: when childNode's weight >= parentNode's weight -> don't bubbleUp
    if (elems[childNode].weight >= elems[parentNode].weight) {
        return;
    }
    /* Test case for swap: do swap childNode & parentNode only when parentNode is not present
     * at the unused cell of priority queue (i.e. index 0)
    */
    if (childNode == 1) return;
    if (parentNode != 0) {
        swap(elems[childNode], elems[parentNode]);
    }
    /* Test case for recrusive case: only when parentNode > 1 recrusive calls can be made.
     * This is done to ensure that bubbleUp doesn't swap 0th and 1st index.
    */
    //if (parentNode > 1) {
        bubbleUp(elems, parentNode, parentNode / 2);
    //}
}

/** Helper function to grow the array when it runs out of space
 * @brief HeapPQueue::grow
 */
void HeapPQueue::grow() {
    allocatedSize *= 2;
    DataPoint* temp = new DataPoint[allocatedSize];

    for (int i = START_INDEX; i <= logicalSize; i++) {
        temp[i] = elems[i];
    }
    delete [] elems;
 ...
 * @brief HeapPQueue::enqueue
 * @param data: input data point to be enqueued
 */
void HeapPQueue::enqueue(const DataPoint& data) {
    // check if allocatedSize needs to be grown when array runs out of space
    if (logicalSize == allocatedSize - START_INDEX) {
        grow();
        //error("Cannot enter more than allocatedSize");
    }
    // Step 1: add data to logicalSize + 1
    int indexAddedTo = logicalSize + START_INDEX;
    elems[indexAddedTo] = data;

    // Step 2: Test for bubbleUp
    bubbleUp(elems, indexAddedTo, indexAddedTo / 2);
...
DataPoint HeapPQueue::peek() const {
    ...
    return elems[START_INDEX];
}

/** Helper function for dequeue
 * @brief HeapPQueue::bubbleDown: recursively checks if the heighest weight element on top of heap can be bubbled down
 * @param elems: pointer to array
 * @param parentNode: highest weight element which we are bubbling down
 * @param childNode: lowest weight childNode to the parentNode
 */
void HeapPQueue::bubbleDown(DataPoint*& elems,
                            int parentNode,
                            int childNode) {
    // Base Case: when parentNode's weight <= childNode's weight
    if (elems[parentNode].weight <= elems[childNode].weight) {
        return;
    }

    // Step 1: swap parent and child node
    swap(elems[parentNode], elems[childNode]);
    // Step 2: update the number of parentNode & childNode after performing step 1 of bubbling down
    int updatedParentNode = childNode;
    int updatedChildNode = findLowestChildNode(elems, updatedParentNode);
    // recursive case
    bubbleDown(elems, updatedParentNode, updatedChildNode);
}

/** Helper function to determine the lowest weight childNode to the current parent node
 * @brief HeapPQueue::findLowestChildNode
 * @param elems: a pointer to array
 * @param parentNode: current node for which we want to determine lowest weight child node
 * @return: lowest weight child node to the parent node
 */
int HeapPQueue::findLowestChildNode(DataPoint*& elems,
                                    int parentNode) {
    if (2*parentNode > logicalSize) {
        return parentNode;
    }
    if (2*parentNode == logicalSize) {
        return 2*parentNode;
    }
    double lowerWeight = min(elems[2*parentNode].weight, elems[(2*parentNode)+1].weight);


    if (lowerWeight == elems[2*parentNode].weight) {
        return (2*parentNode);
    }
 ...
 * @brief HeapPQueue::dequeue
 * @return: a DataPoint with lowest weight present on top of heap (first in the priority queue)
 */
DataPoint HeapPQueue::dequeue() {
    // Step 1: identify element to be dequeued
    DataPoint dequeueElem = peek();

    // Step 2: swap dequeueElem with last element in elems
    swap(elems[START_INDEX], elems[logicalSize]);

    // Step 3: neglect dequeueElem's presence from the array
    logicalSize--;

    // Step 2: Test for bubbleDown
    int parentNode = START_INDEX, childNode;
    // identify lowest weight childNode
    childNode = findLowestChildNode(elems, parentNode);
    // Step 3: check for bubbleDown
    bubbleDown(elems, parentNode, childNode);
...
STUDENT_TEST("Enqueues three elements: negative, positive, zero.") {
    HeapPQueue pq;

    /* Add these items in this order. */
    DataPoint one   = { "First",  0 };
    DataPoint two   = { "Second", 10 };
    DataPoint three = { "Third",  -10 };
