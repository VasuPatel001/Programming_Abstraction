/* File Header: This file contains the function for LinearProbingHashTable
*/
#include "LinearProbingHashTable.h"
using namespace std;

LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    // create a table filled with empty slots and store the hash function for later use
    // determine the # of slots by calling HashFunction<T>::numSlots()
...
/**
 * @brief LinearProbingHashTable::insert: Checks if the elem can be inserted to Linear Probing hash table
 * @param elem: element to be inserted
 * @return: true/false if the elem can be inserted to Linear Probing hash table
 */
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    int i = hashOfElem;
    while (true) {
        // A: slot which is of type empty or tombstone: elem can be inserted
...
/**
 * @brief LinearProbingHashTable::contains: checks if the elem is contained in the Linear Probing hash table
 * @param elem: element to be checked if contained
 * @return: true/false if the elem is present in Linear Probing hash table
 */
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int i = hashOfElem, counter = 0;
    while (true) {
        // A: if hashofElem is "EMPTY" -> return false
        if (elems[i].type == SlotType::EMPTY) return false;

        // B: elem is found in the index hashOfElem
        if (elems[i].type == SlotType::FILLED && elem == elems[i].value) return true;

        /* C: if i reaches allocatedSize - 1, startovr the index with i = -1, since i will
           get increased in the following line*/
        if (i == allocatedSize - 1) i = -1;

        // D: stop looking for elem if checked for all index values of allocated Size
...
/**
 * @brief LinearProbingHashTable::remove: checks if the elem can be removed from Linear Probing hash table
 * @param elem: to be removed
 * @return true/false if the elem can be removed from Linear Probing hash table
 */
bool LinearProbingHashTable::remove(const string& elem) {
    // Step 1: jump to hashOfElem
    int hashOfElem = ourHashFn(elem);
    int i = hashOfElem;
    // Step 2: if elem not contained in Linear Probing hash table -> return false
    if (!contains(elem)) return false;
    while (true) {
        // A: elem found in the Linear Probing hash table
...
void LinearProbingHashTable::printDebugInfo() const {
    for (int i = 0; i < allocatedSize; i++) {
        cout << elems[i].value << ":" << elems[i].type << endl;
    }
}
...
STUDENT_TEST("Test for empty table") {
