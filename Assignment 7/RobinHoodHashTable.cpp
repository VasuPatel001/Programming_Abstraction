/* File header: This file contains code for Robin Hood hash table.
*/
...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    ...
    // initialize elems of length "allocatedSize" and each
...
/**
 * @brief RobinHoodHashTable::insert: this function checks for inserting elem and displacing the elements in Robin hood hash table
 * @param elem: a string to be added to hash table
 * @return true/false if the elem can be inserted to Robin hood hash table
 */
bool RobinHoodHashTable::insert(const string& elem) {
    Slot newSlot;
    newSlot.value = elem, newSlot.distance = 0;
    // Step 1: if the given element is contained in the linearProbingHashTable -> return false;
    if (contains(elem)) return false;

    // Step 2: if numElems == allocatedSize -> return false
    if (numElems == allocatedSize) return false;

    // Step 3: compute the ourHashFn(elem) and see if that space is available, if not add in the next index of elems
    int hashOfElem = ourHashFn(elem);
    int i = hashOfElem;
    while (true) {
        // A: insert when a slot is empty
        if (elems[i].distance == EMPTY_SLOT) {
            elems[i] = newSlot;
            numElems++;
            return true;
        }
        // B: displace an element in array when newSlot's distance is greater than one in the array
        if (newSlot.distance > elems[i].distance) {
            swap(newSlot, elems[i]);
        }
        // C: check for wrapping around the table
        if (i == allocatedSize - 1) i = -1;
        i++, newSlot.distance++;
    }
}

/**
 * @brief RobinHoodHashTable::contains: this function checks if elem is contained in Robin hood hash table
 * @param elem: a string to be checked if present in the hash table
 * @return: true/false if the elem is contained in the Robin hood hash table
 */
bool RobinHoodHashTable::contains(const string& elem) const {
    int hashOfElem = ourHashFn(elem);
    // i keep track of current index value to be checked
    //counter keeps track of number of iterations made while looking for an elem
    int i = hashOfElem, counter = 0, stepsTaken = 0;
    while (true) {
        // A: if hashofElem is "EMPTY" -> return false
        if (elems[i].distance == EMPTY_SLOT) return false;

        // B: elem is found in the index hashOfElem -> return true
        if (elems[i].value == elem) return true;

        // C: steps > elems[i].distance -> return false
        if (stepsTaken > elems[i].distance) return false;

        /* D: wrap around if i reaches allocatedSize - 1, startover the index with i = -1, since i will
           get increased in the following line*/
        if (i == allocatedSize - 1) i = -1;

        // stop looking for elem if checked for all index values of allocated Size
        //if (counter == allocatedSize) return false;
        i++, counter++, stepsTaken++;
    }
}

/** Helper function for remove()
 * @brief RobinHoodHashTable::moveBackward: checks for the possibility of moving elements backward in the array
 * and modifies elems array.
 * @param currentIndex
 */
void RobinHoodHashTable::moveBackward(int currentIndex) {
    int nextIndex;
    while (true) {
        // A: check for wrapping around the array
        if (currentIndex == allocatedSize-1) {
            nextIndex=0;
        } else {
            nextIndex = currentIndex + 1;
        }
        // B: check for stop moving elements backward
        if (elems[nextIndex].distance == EMPTY_SLOT || elems[nextIndex].distance == 0) break;
        // C: swap current & next index slots
        elems[nextIndex].distance--;
        swap(elems[currentIndex], elems[nextIndex]);
        // D: increase the value of current index depending on its position in the array
        if (currentIndex == allocatedSize-1) currentIndex = 0;
        else currentIndex++;
    }
}

/**
 * @brief RobinHoodHashTable::remove: this function checks if elem can be removed from the Robin hood hash table
 * @param elem to be removed from the hash table
 * @return: true/false if the elem can be removed
 */
bool RobinHoodHashTable::remove(const string& elem) {
    // jump to hashOfElem index
    int hashOfElem = ourHashFn(elem);
    int i = hashOfElem;
    // Step 1: check if elem is contained in the hash table
    if (!contains(elem)) return false;
    // Step 2: elem is contained and needs to be removed from hash table
    while (true) {
        // A: remove elem and implement moveBackward()
        ...
        // B: wrap around the table when needed
        if (i == allocatedSize - 1) i = -1;
        i++;
    }
}
...
void RobinHoodHashTable::printDebugInfo() const {
    for (int i = 0; i < allocatedSize; i++) {
        cout << elems[i].value << ":" << elems[i].distance << endl;
    }
}
...
STUDENT_TEST("Test in initially empty table.") {
