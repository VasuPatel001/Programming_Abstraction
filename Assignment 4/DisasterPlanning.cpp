/* File Header: Code in this file, checks the validity of the provided city network whether it is
 * disaster ready and if so, it returns the set of strings which needs to be stockpiled.
*/
 ...
 * @brief canBeMadeDisasterReadyRec: check's if the given network of cities can be made distaster ready,
 *  and returns the set of strings which needs to be stockpiled
 * @param roadNetwork
 * @param numCities
 * @param supplyLocations
 * @param remaining
 * @return true/false for disaster read, if true: returns set of strings which needs to be stockpiled.
 */
...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
    ...
    // Base Case 1: remaining is empty
    if (remaining.isEmpty()) return true;
    // Base Case 2: number of cities remaining to be stockpiled is 0
    if (numCities == 0) return false;

    // Choose city 1
    string firstCity = remaining.first();

    // Case 1: stockpile firstCity
//    if (canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations,
//                               remaining - firstCity - roadNetwork[firstCity])) {
//        supplyLocations.add(firstCity);
//        return true;
//    }

    // Case 2: Stockpile one of the neighbouring city
    for (string neighboringCity: roadNetwork[firstCity] + firstCity) {
        if (canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations,
                                   remaining - neighboringCity - roadNetwork[neighboringCity])) {
            supplyLocations.add(neighboringCity);
            return true;
        }
    }
    return false;
 ...
 * @brief canBeMadeDisasterReady: takes the input parameter from the user and creates Set of remaining cities
 * and passes it on to recursive function
 * @param roadNetwork
 * @param numCities
 * @param supplyLocations
 * @return true/false for disaster read, if true: returns set of strings which needs to be stockpiled.
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if (numCities < 0) {
        error("Non-negative values required");
    }
    Set<string> remaining;
    for (string city: roadNetwork) {
        remaining.add(city);
    }
    return canBeMadeDisasterReadyRec(roadNetwork, numCities, supplyLocations,
                                  remaining);
}
...
STUDENT_TEST("Works for star-connected cities") {
    ...
    Set<string> supply;
    EXPECT(canBeMadeDisasterReady(network, 1, supply));
    EXPECT_EQUAL(supply, {"A"});
}
