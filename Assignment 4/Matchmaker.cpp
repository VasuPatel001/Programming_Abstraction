/* File header: This file contains codes for checking perfectMatching condition
 * and maximumWeightMatching for pairing people.
*/
 ...
 * @brief hasPerfectMatchingRec handles the recursive cases for checking perfectMatching condition &
 * if so returns the set of pairs using outparameter "matching"
 * @param possibleLinks
 * @param matching
 * @param remaining
 * @return true/false for the possiblity of perfect matching, if true: returns the set of pairs
 * using outparameter "matching"
 */
bool hasPerfectMatchingRec(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching,
                           const Set<string>& remaining) {
    //Base Case: There are no people in the remaining set
    if (remaining.isEmpty()) return true;

    // Recursive Case
    // Choose first Person
    string firstPerson = remaining.first();

    // loop over possible connections of firstPerson
    for (string secondPerson: possibleLinks[firstPerson]) {
        if (remaining.contains(secondPerson)) {
            Pair paired(firstPerson, secondPerson);
            if(hasPerfectMatchingRec(possibleLinks,
                                     matching,
                                     remaining - firstPerson - secondPerson)){
                matching.add(paired);
                return true;
            }
        }
 ...
 * @brief hasPerfectMatching: this function checks for the edge cases and creates a set of remaining strings
 * @param possibleLinks
 * @param matching
 * @return true/false for the possiblity of perfect matching, if true: returns the set of pairs
 * using outparameter "matching"
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    //Check if any person doesn't want to pair up, then perfect matching doesn't exist
    ...
    //odd number of people: return false
    if (possibleLinks.size() % 2 != 0) {
        return false;
    }
    return hasPerfectMatchingRec(possibleLinks, matching, remaining);
}


/**
 * @brief valueOf: computes the value of testMatchings under consideration
 * @param possibleLinks
 * @param testMatchings: a set of pairs
 * @return value of possible set of pairs
 */
int valueOf(const Map<string, Map<string, int>>& possibleLinks,
          const Set<Pair>& testMatching) {
    int value = 0;
    for (Pair pair: testMatching) {
        value += possibleLinks[pair.first()][pair.second()];
    }
    return value;
}
...
/** This is a recursive function
 * @brief maximumWeightMatchingRec: finds set of pairs with maximum values
 * @param possibleLinks
 * @param remaining
 * @return a set of pairs with maximum values
 */
Set<Pair> maximumWeightMatchingRec(const Map<string, Map<string,
                                   int>>& possibleLinks,
                                   const Set<string>& remaining) {
    // Base case: if remaining set is empty i.e. not decisions to be made & return empty set of pairs
    if (remaining.isEmpty()) {
        return {};
    }
    // Recursive case
    int maxValue = -1;
    Set<Pair> maxValueSetPair; // set of pairs corresponding to maximum value amongst all the cases considered

    string person1 = remaining.first(); // choose the first person to be considered

    // Decision 1: person1 would never be paired with anyone
    Set<Pair> setPair = maximumWeightMatchingRec(possibleLinks, remaining - person1);
    int valueOfSetPair = valueOf(possibleLinks, setPair);
    if (valueOfSetPair > maxValue) {
        maxValue = valueOfSetPair;
        maxValueSetPair = setPair;
    }

    // Decision 2: person1 would be paired with permissible person2 only if
    // person2 is part of remaining and value of pairing the two is non-negative
    for (string person2: possibleLinks[person1]) {
        if (remaining.contains(person2) && possibleLinks[person1][person2] >= 0) {
            Pair newPair(person1, person2);
            setPair = maximumWeightMatchingRec(possibleLinks, remaining - person1 - person2) + newPair;
            valueOfSetPair = valueOf(possibleLinks, setPair);
            if (valueOfSetPair > maxValue) {
                maxValue = valueOfSetPair;
                maxValueSetPair = setPair;
            }
    ...
    return maxValueSetPair;

}

/** This is wrapper function
 * @brief maximumWeightMatching: a wrapper function which creates a set of remaining strings
 * @param possibleLinks
 * @return a set of pairs with maximum value
 */
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    return maximumWeightMatchingRec(possibleLinks, remaining);
}
...
STUDENT_TEST("hasPerfectMatching doesn't works on a star configuration of people.") {
    /* Here's the world:
     *
     *
     * There are NO perfect matching.
     * Either will work.
     */
...
STUDENT_TEST("maximumWeightMatching: Works for triangular group with negative edges"){
