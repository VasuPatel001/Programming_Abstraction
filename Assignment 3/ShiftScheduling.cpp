/* Function header: This function implements recursion over a set of shifts to
 * find a subset of shifts that has maximum value and satisfying the criteria
 * of maxHours allowed and non-overlaping shifts.
*/

#include "ShiftScheduling.h"
using namespace std;

/** Checks whether a given shift overlaps with set of selected shifts
 * @brief isOverlapWith
 * @param shift
 * @param selected
 * @return true if a shift overlaps with selected set of shift, if not false
 */
bool isOverlapWith(const Shift& shift,
                   const Set<Shift>& selected) {
    for (Shift elm: selected) {
        if (overlapsWith(shift, elm)) {
            return true;
        }
    }
    return false;
...
/** Evaluates the total value of seclted set of shifts
 * @brief valueOfselected
 * @param selected
 * @return int value of selected shifts
 */
int valueOfSelected(const Set<Shift>& selected) {
    int value = 0;
    for (Shift shift: selected){
        value += valueOf(shift);
    }
    return value;
}

/** Recursive function to find the highest value provided a set of
 *  remaining and selected shifts.
 * @brief highestValueScheduleRec
 * @param remaining
 * @param selected
 * @param totalHours
 * @param usedHours
 * @return a set of shifts with highest value.
 */
Set <Shift> highestValueScheduleRec(const Set<Shift>& remaining,
                                    const Set<Shift>& selected,
                                    int totalHours, int usedHours) {
    // Base case when the remaining set of shifts if empty
    if (remaining.isEmpty()) {
        return selected;
    } else {
        // Variable initialization
        Set<Shift> with, without;
        // Pick a random shift from remaining set of shifts
        Shift shift = remaining.first();

        // Case when shift is selected (i.e. Check if the shift doesn't overlap and usedHours <= totalHours)
        if ((! isOverlapWith(shift, selected)) && (usedHours + lengthOf(shift) <= totalHours)) {
            with = highestValueScheduleRec(remaining - shift,
                                           selected + shift,
                                           totalHours, usedHours + lengthOf(shift));
        }
        // Case when shift is not selected
        without = highestValueScheduleRec(remaining - shift,
                                          selected,
                                          totalHours, usedHours);

        // Find value associated "with" & "without" set of shifts
        int valueWith = valueOfSelected(with);
        int valueWithout = valueOfSelected(without);

        // select a shift with a hightest value
...
/** "highestValueScheduleFor" is Wrapper function
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> selected;
    return highestValueScheduleRec(shifts, selected, maxHours, 0);
}
...
STUDENT_TEST("Handles no shifts & No availability") {
    EXPECT_EQUAL(highestValueScheduleFor({}, 0).size(), 0);
}
