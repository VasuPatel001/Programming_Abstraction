 ...
 * Code to determine the most optimum schedule of shift to
 * maximize value
 */
#include "ShiftScheduling.h"
using namespace std;

/* Helper function to determine if there is a conflict of schedule
 * between a given shift and the already scheduled shifts
 */
bool noConflict(Shift shift, Set<Shift> shiftsTaken) {
    for (Shift takenShift: shiftsTaken){
        if (overlapsWith(shift, takenShift)){
            return false;
        }
    }
    return true;
...
/* Helper function to calculate value associated with a
 * given schedule of shifts
 */
...
/* Recursive function to determine a schedule of shifts to maximize value
 * Input: sets of shifts remaining and already scheduled, number of hours scheduled and
 * maximum permissible hours
 * Output: Optimum schedule of shifts
 */
Set<Shift> highestValueScheduleForRec(Set<Shift> shiftsRemaining, Set<Shift> shiftsTaken,
                                      int takenHours, int maxHours) {
    if (shiftsRemaining.isEmpty()) { // base case: if all remaining shifts are exhausted
        return shiftsTaken;
    } else { // recursive case
        Set<Shift> scheduleWith;
        Set<Shift> scheduleWithout;

        Shift shift = shiftsRemaining.first();
        shiftsRemaining.remove(shift);
        // determine compatibility of chosen shift wrt maxHours and conflict with already chosen shifts
        if (lengthOf(shift) + takenHours <= maxHours && noConflict(shift, shiftsTaken)){
            scheduleWith = highestValueScheduleForRec(shiftsRemaining, shiftsTaken + shift,
                                                      takenHours + lengthOf(shift), maxHours);
        }
        scheduleWithout = highestValueScheduleForRec(shiftsRemaining, shiftsTaken, takenHours, maxHours);
        // value associated with the two schedules
        int valueWith = valueOfSchedule(scheduleWith);
        int valueWithout = valueOfSchedule(scheduleWithout);

        if (valueWith > valueWithout){
            return scheduleWith;
        } else {
            return scheduleWithout;
        }
    }
}

/* Wrapper function to call the recursive function to determine the optimum shifts schedule
 * Input: set of shifts, max available hours
 * Ouput: optimum schedule
 */
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0){
        error("Max hours must be non-negative");
    }
    Set<Shift> shiftsDone;
    int takenHours = 0;
    return highestValueScheduleForRec(shifts, shiftsDone, takenHours, maxHours);
}
...
STUDENT_TEST("Returns empty schedule if all slots are larger than maximum amount of hours."){
    Set<Shift> shifts = {
        { Day::MONDAY,    10, 17, 21 },
    };
    Set<Shift> test;
    EXPECT_EQUAL(highestValueScheduleFor(shifts, 2), test);
}
