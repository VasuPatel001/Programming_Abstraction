/*File header: This file contains code to draw a self similar triangles,
 * also called Sierpinski triangle.
*/
...
/** Recursive function for drawing self similar sierpinski triangles
...
void drawSierpinskiTriangle(GWindow& window,
                            ...
                            double x2, double y2,
                            int order) {
    // Raise exception for order < 0
    if (order < 0) {
        error("Please enter order >=0 ");
    }
    // Base Case: Order 0 draw a single black traingle
    else if (order == 0) {
        drawTriangle(window,
                     x0, y0,
                     x1, y1,
